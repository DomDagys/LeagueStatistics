...
class ProfilePage extends React.Component {
  ...
  getProfileData(summonerName, region) {
    ...
    leagueService
      .getRankedStats(summonerName, region)
      .then((leagueData) => this.setState({ leagueData: leagueData }));

    quickstatsService
      .getStatistics(summonerName, region)
    ...
    return (
      <div>
        ...
        <div className="input-group mb-3">
          ...
          <button onClick={this.handleLive}>Live Game</button>
        </div>
        {this.state.summonerData !== null && (
          <SummonerProfile summonerData={this.state.summonerData} />
        )}
        {this.state.leagueData !== null && (
          <LeagueRanks leagueData={this.state.leagueData} />
        )}
